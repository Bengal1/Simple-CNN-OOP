...
void SimpleCNN::trainSimpleCNN(MNISTLoader& dataLoader, const size_t epochs) {
    ...
    for (size_t epoch = 1; epoch <= epochs; ++epoch) {
        ...
        _trainingStats[TRAIN_LOSS][epoch - 1] = _trainEpoch(trainImages, oneHotTrainLabels, trainOutput);
        _trainingStats[TRAIN_ACCURACY][epoch - 1] = _accuracyCalculation(trainOutput, oneHotTrainLabels);

        std::vector<Eigen::VectorXd> validationOutput(numValidationImages, Eigen::VectorXd(_classes));
        _trainingStats[VALIDATION_LOSS][epoch - 1] = _validateEpoch(validationImages, oneHotValidationLabels, validationOutput);
        _trainingStats[VALIDATION_ACCURACY][epoch - 1] = _accuracyCalculation(validationOutput, oneHotValidationLabels);
