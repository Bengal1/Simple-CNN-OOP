...
Convolution2D::Convolution2D(size_t inputHeight, size_t inputWidth, size_t inputChannels,
    ...
    size_t batchSize, size_t stride, size_t padding)
...
std::vector<Eigen::MatrixXd> Convolution2D::backward(const std::vector<Eigen::MatrixXd>& dLoss_dOutput) {
...
void Convolution2D::_initializeFilters() {
    ...
    std::mt19937 gen(rd());
    std::normal_distribution<double> dist(0.0, std::sqrt(2.0 / (_kernelSize * _kernelSize * _inputChannels)));

    for (size_t f = 0; f < _numFilters; ++f)
        for (size_t c = 0; c < _inputChannels; ++c)
            _filters[f][c] = Eigen::MatrixXd::NullaryExpr(_kernelSize, _kernelSize, [&]() { return dist(gen); });
...
void Convolution2D::_validateInputParameters() const {
    if (_inputHeight == 0 || _inputWidth == 0 || _inputChannels == 0 ||
        _numFilters == 0 || _kernelSize == 0 || _stride == 0 || _batchSize == 0)
...
const Eigen::MatrixXd Convolution2D::_padWithZeros(const Eigen::MatrixXd& input, size_t promptPadding) const {
...
const std::vector<Eigen::MatrixXd> Convolution2D::_padWithZeros(const std::vector<Eigen::MatrixXd>& input, size_t promptPadding) const {
    std::vector<Eigen::MatrixXd> padded;
    padded.reserve(input.size());
    for (const auto& mat : input)
        padded.push_back(_padWithZeros(mat, promptPadding));
    return padded;
}

Eigen::MatrixXd Convolution2D::_Convolve2D(const Eigen::MatrixXd& input, const Eigen::MatrixXd& kernel, size_t padding) const {
    ...
    for (size_t i = 0; i < outH; ++i)
        for (size_t j = 0; j < outW; ++j)
            result(i, j) = (paddedInput.block(i * _stride, j * _stride, kernel.rows(), kernel.cols()).cwiseProduct(kernel)).sum();
