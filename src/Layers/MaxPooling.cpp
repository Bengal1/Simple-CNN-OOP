...
MaxPooling::MaxPooling(size_t inputHeight, size_t inputWidth, size_t inputChannels,
    size_t poolSize, size_t batchSize, size_t stride)
    : _inputHeight(inputHeight), _inputWidth(inputWidth), _inputChannels(inputChannels),
    _kernelSize(poolSize), _stride(stride), _batchSize(batchSize) {
    _validate();
    _initialize();
}
    ...
    for (size_t c = 0; c < _inputChannels; ++c) {
        for (size_t h = 0; h < _outputHeight; ++h) {
            for (size_t w = 0; w < _outputWidth; ++w) {
                _output[c](h, w) = input[c].block(h * _stride, w * _stride, _kernelSize, _kernelSize)
                    .maxCoeff(&row, &col);
...
std::vector<std::vector<Eigen::MatrixXd>> MaxPooling::forwardBatch(
    ...
    for (size_t b = 0; b < _batchSize; ++b) {
        for (size_t c = 0; c < _inputChannels; ++c) {
            for (size_t h = 0; h < _outputHeight; ++h) {
                for (size_t w = 0; w < _outputWidth; ++w) {
                    _outputBatch[b][c](h, w) = inputBatch[b][c].block(h * _stride, w * _stride, _kernelSize, _kernelSize)
                        .maxCoeff(&row, &col);
...
void MaxPooling::_initialize() {
    ...
    if (_batchSize == 1) {
        ...
        _outputBatch.assign(_batchSize, std::vector<Eigen::MatrixXd>(
...
void MaxPooling::_getDataLocation(size_t& dataChannel, size_t& dataRow,
...
void MaxPooling::_getDataLocation(size_t& dataBatch, size_t& dataChannel,
